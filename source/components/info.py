...
class Info:
    ...
    def create_state_labels(self):
        ...
        if self.state == 'main_menu':
            ...
            self.state_labels.append((self.create_label('X    {}'.format(self.game_info['lives'])), (380, 280))) #{}占位符，format替换占位符
            self.player_image = tools.get_image(setup.GRAPHICS['mario_bros'], 178, 32,
                                                12, 16, (0, 0, 0), C.BG_MULTI)
    ...
    def create_label(self,label,size=40,width_scale=1.25,height_scale=1):
        font = pygame.font.SysFont(C.FONT,size)#创建字体对象
        label_image = font.render(label,1,(255,255,255))#将文本渲染成图像,lable是要显示的文本内容
